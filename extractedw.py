...
def process_file(file_path, validator, label):
    output_filename = f"extracted_{label.replace(' ', '_')}.txt"

    try:
        with open(file_path, 'r', encoding='utf-8') as f, open(output_filename, 'w', encoding='utf-8') as out:
            for line_num, line in enumerate(f, 1):
                ...
                if is_valid:
                    out.write(line + '\n')
        print(f"\n✅ Valid {label} entries have been saved to {output_filename}\n")
...
def main():
    ...
    while True:
        ...
        if 1 <= choice <= 5:
            ...
            print("Thank you for using the Regex Pattern Finder & Checker. Goodbye!")
            break
        else:
